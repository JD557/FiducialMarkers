...
public class Work {
    ...
    private final double UPDATE = 0.5;
    
    private boolean detected = false;
    private Point[] pt = new Point[4];
    
	public Work(Mat frame, Point[] pt){
		capture=frame;
	...
	public Point[] call(Mat imageHSV, Mat redImage, Mat greenImage,	Mat imgThr1, Mat imgThr2, Mat[] regions) {
		if(detected)
			return track(capture,imageHSV,redImage,greenImage,imgThr1,imgThr2,regions);
		else
			return detect(capture, imageHSV, redImage, greenImage, imgThr1, imgThr2);	
	}    
	...
	private void extractRed(Mat imageHSV, Mat imgThr1, Mat imgThr2, Mat redImage) {
	...
	private void extractGreen(Mat imageHSV, Mat greenImage) {
	        Core.inRange(imageHSV,new Scalar(45,20,20),new Scalar(75,255,255),greenImage);
	        Imgproc.morphologyEx(greenImage, greenImage, Imgproc.MORPH_CLOSE, new Mat());
	}
	
	public Point[] track(Mat capture,Mat imageHSV,Mat redImage,Mat greenImage,Mat imgThr1,Mat imgThr2,Mat[] regions){
		Imgproc.cvtColor(capture, imageHSV, Imgproc.COLOR_RGB2HSV);
        for (int i=0;i<4;++i) {
        ...
        extractRed(regions[0],imgThr1,imgThr2,redImage);
        ...
        for (int i=1;i<4;++i) {
                extractGreen(regions[i],greenImage);
	...
	public Point[] detect(Mat capture,Mat  imageHSV,Mat  redImage,Mat  greenImage,Mat  imgThr1,Mat  imgThr2){
		Imgproc.cvtColor(capture, imageHSV, Imgproc.COLOR_RGB2HSV);
        Imgproc.resize(imageHSV, imageHSV, new Size(),1/SCALE_FACTOR,1/SCALE_FACTOR,Imgproc.INTER_NEAREST);
        extractRed(imageHSV,imgThr1,imgThr2,redImage);
        extractGreen(imageHSV,greenImage);
        //extractBlue();
